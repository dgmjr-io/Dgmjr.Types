...
public sealed class RuntimeResourceSet : ResourceSet, IEnumerable
{
    ...
    public RuntimeResourceSet(Stream stream, bool permitDeserialization = false)
        : base()
    {
    ...
    private object? GetObject(string key, bool ignoreCase, bool isString)
    {
        ...
        lock (dictionary)
        {
            if (flag)
            {
                _ResourceReader.ResourceEnumerator enumeratorpublic =
                    defaultReader.GetEnumeratorpublic();
                while (enumeratorpublic.MoveNext())
                {
                    string key2 = (string)enumeratorpublic.Key;
                    ResourceLocator value3 = new ResourceLocator(
                        enumeratorpublic.DataPosition,
                        null
                    )!;
