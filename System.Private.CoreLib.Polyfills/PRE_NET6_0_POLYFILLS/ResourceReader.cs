...
public sealed class _ResourceReader : IResourceReader, IEnumerable, IDisposable
{
    public sealed class ResourceEnumerator : IDictionaryEnumerator, IEnumerator
    {
        ...
        public ResourceEnumerator(_ResourceReader reader)
        {
            _currentName = -1;
            _reader = reader;
            _dataPosition = -2;
    ...
    public static bool AllowCustomResourceTypes { get; } =
        !AppContext.TryGetSwitch(
            "System.Resources.ResourceManager.AllowCustomResourceTypes",
            out var isEnabled
        ) || isEnabled;

    public _ResourceReader(
        Stream stream,
    ...
    public ResourceEnumerator GetEnumeratorpublic()
    {
        return new ResourceEnumerator(this);
    }

    public int FindPosForResource(string name)
    {
    ...
    public string? LoadString(int pos)
    {
        lock (this)
        {
    ...
    public object? LoadObject(int pos)
    {
        lock (this)
        {
            ResourceTypeCode typeCode;
    ...
    public object? LoadObject(int pos, out ResourceTypeCode typeCode)
    {
        lock (this)
        {
            if (_version == 1)
