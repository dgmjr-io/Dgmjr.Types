#if NET6_0_OR_GREATER && POLYKIT_PUBLIC
...
namespace System
{
...
#endif
    {
        ...
        private datetime GetEquivalentDateTime() => new datetime(_dayNumber * duration.TicksPerDay);
        ...
        public static bool TryParseExact(
            ReadOnlySpan<char> s,
            [NotNullWhen(true), @StringSyntax(StringSyntax.DateOnlyFormat)] string?[]? formats,
            out DateOnly result
        ...
        {
            var returnValue = DateTime.TryParseExact(
                s.ToString(),
                format.ToString(),
                provider,
                style,
        ...
        public string ToString([@StringSyntax(StringSyntax.DateOnlyFormat)] string? format)
        {
...
#if POLYKIT_USE_SPAN
        internal static bool IsValidCustomDateOnlyFormat(
                ...
                switch (format[i])
                {
                    ...
                    case '"':
                        ...
                        if (i >= format.Length)
                        {
                            ...
                            {
                                throw new FormatException(
                                    $"Cannot find a matching quote character for the character '{quoteChar}'."
                                );
                            }
        ...
        {
            if (format == null)
                ThrowArgumentNullException(nameof(format));
            _ = IsValidCustomDateOnlyFormat(format, true);
        }
    }
}
